...
void alg_disparo_profundidadeN(Grafo *G, int j){
    ...
    for(i = j; i < G -> quantidade_vertices; i++){
        if(cor[i] == BRANCO)
            busca_profundidadeN(G, i, cor);
    }
     printf("\n\n---------------------------------------------------------------------------\n");
}

void busca_profundidadeN(Grafo *G, int i, int *cor){ //visitaProfundidade de forma nao conexa
    cor[i] = AMARELO;
    //printf("%d", cor[i]); //printa o vetor de visitados
    printf("[%d]  ", i); //printa a ordem de visita da busca
    //mostra_adjacentes(G, i); //printa os vertices adjacentes a i
    ADJACENCIA *aux = G -> adj[i].cab;
    //printf("[%d]\t",aux -> vertice); //printa o vertice que sera usado na prox. chamada pela funcao

    while(aux){
        if(cor[aux -> vertice] == BRANCO){
            busca_profundidadeN(G, aux -> vertice, cor);
        }
        aux = aux -> prox;
    }
    cor[i] = VERMELHO;
