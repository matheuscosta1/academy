...
void busca_profundidadeN(Grafo *G, int vertice, int *cor){ //visitaProfundidade de forma nao conexa
    int i=0;
    cor[vertice] = 1;
    printf("[%d]\t", vertice);
    int cont=0;
    ...
    for(i=0; i<G->quantidade_vertices; i++){
       // vertice = i;
        if(G->aresta[vertice][i]!=0 && cor[i]==0){

                //printf("---%d -> %d = %d--\n", vertice, i, G->aresta[vertice][i]);
                busca_profundidadeN(G, i, cor);
                //G->aresta[vertice] = i;
