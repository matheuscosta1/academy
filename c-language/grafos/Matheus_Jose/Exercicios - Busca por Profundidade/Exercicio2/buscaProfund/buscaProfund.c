...
int remover_aresta(Grafo *G, int vertice1, int vertice2){
    ...
    while(aux != NULL){
        if(aux -> vertice == vertice2){ // se a adjacencia do vertice 1 é igual a do vertice 2, entaooooo, removo essa joça!
            if (ant != NULL){
                ant -> prox = aux->prox;
            }else{
...
void alg_disparo_profundidade(Grafo *G, int j){
    int cor[G -> quantidade_vertices];
    int i;
    for(i = 0; i < G -> quantidade_vertices; i++)
        cor[i] = BRANCO;
    printf("\n---------------------------------------------------------------------------\n");
    printf("Ordem de visita da busca        Vertice que sera usado na proxima chamada da funcao\n\n");
    busca_profundidade(G, j, cor);
    printf("\n---------------------------------------------------------------------------\n");
    printf("\n---------------------------------------------------------------------------\n");
    printf("Vetor de visitados\n\n");
    for(i=j; i <G->quantidade_vertices;i++)
        printf("%d\t", cor[i]); //printa o vetor de visitados
    printf("\n\n---------------------------------------------------------------------------");
    printf("\n\n");

    printf("\---------------------------------------------------------------------------");
    printf("\nMostrar adjacentes do vertice atual\n\n\n");
    for(i=0; i <G->quantidade_vertices;i++)
...
void busca_profundidade(Grafo *G, int i, int *cor){ //visitaProfundidade de forma nao conexa
    cor[i] = AMARELO;
    printf("\t[%d]\t", i); //printa a ordem de visita da busca

    ADJACENCIA *aux = G -> adj[i].cab;
    printf("\t |");
    printf("\t\t[%d]\t\n", aux -> vertice); //printa o vertice que sera usado na prox. chamada pela funcao
    while(aux){
        if(cor[aux -> vertice] == BRANCO){
            busca_profundidade(G, aux -> vertice, cor);
        }
        aux = aux -> prox;
    }
}
