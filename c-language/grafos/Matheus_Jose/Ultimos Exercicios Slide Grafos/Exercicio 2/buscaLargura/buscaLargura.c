...
void busca_largura(Grafo *G, int vertice){
    ...
    if (ultimo == G->quantidade_vertices-1){
        printf("\n\nO vertice [%d] eh conectado a todos os demais", vertice);
    } else {
        printf("\n\nO vertice [%d] nao eh conectado aos demais\n\n", vertice);
        for(i=ultimo+1; i<G->quantidade_vertices;i++){
            printf("[%d]\t", i);
        }
    }
}
