...
int Insere_Aresta(Grafo* g,int v1,int v2)
{
  ...
  if(n==NULL)return -1;
     ...
     n->vertice=v2;
     //colocando v2 na lista v1
     n->prox=g->aresta[v1];
     g->aresta[v1]=n;

     n=(No*)malloc(sizeof(No));//alocando um novo no
  if(n==NULL)return -1;

     n->vertice=v1;
     //colocando v1 na lista v2
     n->prox=g->aresta[v2];
     g->aresta[v2]=n;
        ...
        atual = vertice;
           ...
           while(aux!=NULL)
           {
                   if(!visitados[vertice])
                   {
                       ...
                       if((*Distancia)[vertice]>1+(*Distancia)[atual])
                       {
                           (*Distancia)[vertice]=1+(*Distancia)[atual];
